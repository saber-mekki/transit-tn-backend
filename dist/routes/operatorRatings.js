...
exports.router.get('/', async (req, res) => {
    try {
        const operators = await db_1.default.user.findMany({
            where: { role: 'OPERATOR' },
            include: {
...
exports.router.get('/leaderboard', async (req, res) => {
    try {
        const operators = await db_1.default.user.findMany({
            where: { role: 'OPERATOR' },
...
exports.router.get('/:id/ratings', async (req, res) => {
    try {
        const ratings = await db_1.default.operatorRating.findMany({
...
exports.router.post('/:id/rate', auth_1.authenticate, async (req, res) => {
    ...
    try {
        const operator = await db_1.default.user.findUnique({ where: { id: req.params.id } });
        if (!operator)
        ...
        const existing = await db_1.default.operatorRating.findFirst({
            where: { operatorId: req.params.id, userId: req.user.id }
        });
        if (existing) {
            const updated = await db_1.default.operatorRating.update({
                where: { id: existing.id },
                data: { score, comment: comment || null }
            });
            return res.json(updated);
        }
        const rating = await db_1.default.operatorRating.create({
